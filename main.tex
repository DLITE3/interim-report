...
\section{先行研究}
\noindent
\space うお峰野は視覚障害者が散策する目的として、気候や季節を見計らって気分転換がてらに行う要因があると述べている(峰野　2013)。しかし視覚障がい者は目での観察が不可能なため、気分転換の効果が軽減してしまうのではないかと考える。これは音を聞くことが出来ない聴覚障がい者にも言えるのではないかと考えた。
\section{研究動機}
\noindent
hogehoge
\section{目的}
...
\subsection{ソフトウェア}
\noindent
\space 後期は生成AIのAPIを使って、本格的にソフトウェアの制作に取り掛かる。raspberry piと連携をさせ、実用できる段階まで制作する。また中間発表会の質問にあったように、実際に生成AIで生成された音楽やアートが、元の景色や音を表現するものとして妥当性があるかを評価するため、アンケートなどをとり、実用性を高めていきたい。
\end{document}